...
SFSymbolsKit is a tiny ***Swift*** package that provides extensions to `String`, `UIImage` and `NSImage` to make using **SFSymbols** easy peasy. The extensions and the accompanying `Enum` are all generated using the python scripts included in this package along with a list of all of the `SFSymbols` names — generated from <!-- sfsymbols-version:start -->Apple's **SFSymbols 7.2** catalog<!-- sfsymbols-version:end -->. 
